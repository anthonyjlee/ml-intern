...
def _is_transient_error(error: Exception) -> bool:
    ...
    if isinstance(error, EmptyLLMChoicesError):
        return True
    ...
    for _llm_attempt in range(_MAX_LLM_RETRIES):
        try:
            ...
            try:
                first_choice(
                    response,
                    model_name=llm_params.get("model", session.config.model_name),
                    operation="Non-streaming LLM response",
                )
                break
            except EmptyLLMChoicesError as e:
                empty_response_error = e
            _delay = _retry_delay_for(empty_response_error, _llm_attempt)
            if _llm_attempt < _MAX_LLM_RETRIES - 1 and _delay is not None:
                logger.warning(
                    "Empty LLM response (attempt %d/%d): %s - retrying in %ds",
                    _llm_attempt + 1, _MAX_LLM_RETRIES, empty_response_error, _delay,
                )
                await session.send_event(Event(
                    event_type="tool_log",
                    data={"tool": "system", "log": f"LLM returned an empty response, retrying in {_delay}s..."},
                ))
                await asyncio.sleep(_delay)
                continue
